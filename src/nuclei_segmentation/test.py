...
def test_model(is_workflow=False):
    ...
    for _ in tqdm(range(CONFIG['tries_num']), desc='Try'):
        ...
        for exp_name in test_experiment_types:
            ...
            cnr_val = mean_cells_ratio(gt=np.array(test_labels)[exp_img_inds], pred=np.array(masks)[exp_img_inds])
    ...
    for exp_name in test_experiment_types:
        np.save(os.path.join(str(out_dir), f'ap_05_{exp_name}.npy'), np.array(ap_05[exp_name]))
        np.save(os.path.join(str(out_dir), f'ap_075_{exp_name}.npy'), np.array(ap_075[exp_name]))
        np.save(os.path.join(str(out_dir), f'cnr_{exp_name}.npy'), np.array(cnr[exp_name]))
...
def test_fsl(is_workflow=False):
    ...
    np.save(os.path.join(str(out_dir), 'ap_05_all.npy'), np.array(test_metrics['all']['ap_0.5']))
    np.save(os.path.join(str(out_dir), 'ap_075_all.npy'), np.array(test_metrics['all']['ap_0.75']))
    np.save(os.path.join(str(out_dir), 'cnr_all.npy'), np.array(test_metrics['all']['cell_ratio']))

    for exp_name in test_experiment_types:
        np.save(os.path.join(str(out_dir), f'ap_05_{exp_name}.npy'), np.array(test_metrics[exp_name]['ap_0.5']))
        np.save(os.path.join(str(out_dir), f'ap_075_{exp_name}.npy'), np.array(test_metrics[exp_name]['ap_0.75']))
        np.save(os.path.join(str(out_dir), f'cnr_{exp_name}.npy'), np.array(test_metrics[exp_name]['cell_ratio']))
