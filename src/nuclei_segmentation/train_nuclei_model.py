...
def train_nuclei_model():
    ...
    if CONFIG['not_treated_exp']:
        train_dirs_list.append(os.path.join(data_path, 'train_by_exps', 'Nthyori,exp3,cond3'))
        train_dirs_list.append(os.path.join(data_path, 'train_by_exps', 'Nthyori,exp5,cond1a,control'))
    else:  
        train_dirs_list.append(os.path.join(data_path, 'train_by_exps', 'Nthyori,exp3,cond3'))
        train_dirs_list.append(os.path.join(data_path, 'train_by_exps', 'Nthyori,exp5,cond1a,control'))
        train_dirs_list.append(os.path.join(data_path, 'train_by_exps', 'Nthyori,exp4,cond3'))
    ...
    print('Train')
    new_model_path = model.train(list(train_data), list(train_labels), 
                                test_data=None,
                                test_labels=None,
                                channels=channels, 
    ...
    with mlflow.start_run(run_name=template_name) as run:
        ...
        shutil.copy(new_model_path, os.path.join(artifacts_model_path, 'cellpose_thyroid'))

        code_path = Path(__file__).parent
        config_path = code_path / 'configs/train_config.yaml'
        shutil.copy(config_path, os.path.join(artifacts_folder, 'configs/train_config.yaml'))
