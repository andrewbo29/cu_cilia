...
wandbc = WeightsAndBiasesCallback(
    wandb_kwargs={"project": project_name}, as_multirun=True
)
...
def objective(trial):
    ...
    params["nimg_per_epoch"] = trial.suggest_categorical(
        "nimg_per_epoch", [5, 10, 20, 30, 40, 45, 50]
    )
    ...
    for _ in range(kf.n_splits):
        ...
        masks = model.eval(
            list(val_data_fold), channels=[chan, chan2], diameter=diameter
        )[0]
...
def objective_on_test(trial):
    ...
    params["nimg_per_epoch"] = trial.suggest_categorical(
        ...
        [
            train_dataset_len,
            int(train_dataset_len * 1.5),
            int(train_dataset_len * 2),
            int(train_dataset_len * 3),
        ],
    )
    params["batch_size"] = trial.suggest_categorical("batch_size", [1, 4, 8, 16, 32])
    ...
    for _ in range(kf.n_splits):
        ...
        for __ in range(3):
            ...
            for j in range(len(train_inds_list)):
                support_ind = np.random.choice(
                    train_inds_list[j], params["n_support"], replace=False
                )
